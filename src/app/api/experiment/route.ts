...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ error: `Failed to fetch experiments: ${err}` }, { status: 500 });
  }
}
  ...
  try {
    ...
    if (action === 'create') {
      ...
      if (!posts || !Array.isArray(posts) || posts.length < 3)
        return NextResponse.json({ error: 'Not enough posts' }, { status: 400 });
      ...
      if (!exp) return NextResponse.json({ error: 'Failed to create experiment' }, { status: 500 });
      ...
      for (const item of withTemplates) {
        const post = await insert('posts', {
          ...
          tweet_id: item.post.postId, post_url: item.post.postUrl, content: item.post.content,
